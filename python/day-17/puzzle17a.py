...
def expand_grid(grid):
    ...
    n_z = len(grid[0][0]) + 2
    #print(n, n_z)
    bigger_grid = [[['.' for k in range(n_z)] for j in range(n)] for i in range(n)]
    for x in range(1, n-1):
        for y in range(1, n-1):
            for z in range(1, n_z-1):
                #print(f'{(x,y,z)} is {grid[y-1][x-1]}')
                if n == 5:
                    bigger_grid[x][y][z] = grid[y-1][x-1]
                else:
                    #print(z)
                    bigger_grid[x][y][z] = grid[y-1][x-1][z-1]
    return bigger_grid
        ...
        for y in range(n):
            for z in range(n_z):
                ...
                for i in range(-1, 2):
                    for j in range(-1, 2):
                        for k in range(-1, 2):
                            if 0 <= x+i < n and 0 <= y+j < n and 0 <= z+k < n_z and not(i==0 and j==0 and k==0):
                                neighbours.append(grid[x+i][y+j][z+k])
